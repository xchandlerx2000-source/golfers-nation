...
function createDiscoveryCourseRecord(course = {}, detailShard = "unknown") {
  ...
  return {
    ...
    metadata: {
      ...
      capabilities: course?.metadata?.capabilities || {},
      teeTimes: course?.metadata?.teeTimes || course?.metadata?.booking || course?.metadata?.capabilities?.teeTimes || {},
      booking: course?.metadata?.booking || course?.metadata?.teeTimes || course?.metadata?.capabilities?.teeTimes || {},
      onCourseServices: course?.metadata?.onCourseServices || course?.metadata?.serviceCapabilities || course?.metadata?.capabilities?.onCourseServices || {},
      serviceCapabilities: course?.metadata?.serviceCapabilities || course?.metadata?.onCourseServices || course?.metadata?.capabilities?.onCourseServices || {},
